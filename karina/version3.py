...
LASER_IMAGE_PATH = os.path.join(PLAY_PATH, "laser.png")
...
def draw_screen(line_list, lase):
    ...
    for acceleration in accelerations:
        screen.blit(acceleration[2], (acceleration[0], acceleration[1]))

    for deceleration in decelerations:
        screen.blit(deceleration[2], (deceleration[0], deceleration[1]))
    ...
    for coin in coins:
        screen.blit(coin_image, (coin[0] - coin_image.get_width() // 2, coin[1] - coin_image.get_height() // 2))  # Відображаємо зображення монеткиfor coin in coins:
        pygame.draw.circle(screen, 'gold', coin, 10)

    for acceleration in accelerations:
        screen.blit(acceleration.image, (acceleration[0] - acceleration_image.get_width() // 2, acceleration[1] - acceleration_image.get_height() // 2))  # Відображаємо зображення монеткиfor coin in coins:
        pygame.draw.acceleration

    for deceleration in decelerations:
        screen.blit(deceleration_image, (deceleration[0] - deceleration_image.get_width() // 2, deceleration[1] - deceleration_image.get_height() // 2))  # Відображаємо зображення монеткиfor coin in coins:
        pygame.draw.deceleration

    # Відображення лазера
    screen.blit(laser_image, (laser[0][0], laser[0][1]))
...
acceleration_image = pygame.image.load(ACCELERATION_IMAGE_PATH).convert_alpha()
deceleration_image = pygame.image.load(DECELERATION_IMAGE_PATH).convert_alpha()
laser_image = pygame.transform.scale(pygame.image.load(LASER_IMAGE_PATH).convert_alpha(), (int(pygame.image.load(LASER_IMAGE_PATH).get_width() / 3), int(pygame.image.load(LASER_IMAGE_PATH).get_height() / 3)))  # Завантажуємо зменшене зображення лазера
